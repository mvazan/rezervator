...
void main() {
  ...
  group('pointsLabel', () {
    ...
    test('half points use a Czech comma',
        () => expect(pointsLabel(2.5, 5.5), '2,5 : 5,5'));
    test('neither side has a result yet',
  ...
  group('formatLabel', () {
    ...
    test('TEAMS_OF_4 alone (Czech 2–4 agreement)',
        () => expect(formatLabel('TEAMS_OF_4', ''), '4 hráči'));
    test('unknown match type is omitted, discipline still shows',
        () => expect(formatLabel('', 'T100'), '100 HS'));
    test('both unknown is empty',
        () => expect(formatLabel('', ''), ''));
  });
  ...
  group('isLive', () {
    ...
    test('scheduled 30 minutes before start is live (refresh window)', () {
      expect(
        isLive(slot(), result('scheduled'), start.subtract(const Duration(minutes: 30))),
        isTrue,
      );
    });
    ...
    test('scheduled 2 hours before start is not live yet', () {
      expect(
        isLive(slot(), result('scheduled'), start.subtract(const Duration(hours: 2))),
        isFalse,
      );
    });
    ...
    test('no result row yet, 1 hour after start, is live (treated as scheduled)', () {
      expect(
  ...
  group('resultsTimeline', () {
    ...
    test('only federation matches with no parent, days ascending, sorted by start then title',
        () {
    ...
    test('mineOnly honours followed teams', () {
      ...
      expect(days.expand((d) => d.matches).map((m) => m.id).toSet(), {'a', 'c'});
    });
  ...
  group('todayIndex', () {
    ...
    test('first day >= today', () {
      final days = [dayOf('2026-09-20'), dayOf('2026-09-27'), dayOf('2026-10-04')];
  ...
  group('venuesMatching', () {
    ...
    final venues = [
      ...
      venue(name: 'TJ Sokol Husovice', address: 'Dukelská 1, Brno', clubs: const ['TJ Sokol Husovice']),
      venue(name: 'Áčko Blansko'),
    ];

    test('empty query returns everything, Czech-sorted by name', () {
      final result = venuesMatching(venues, '');
      expect(result.map((v) => v.name), ['Áčko Blansko', 'KS Devítka Brno', 'TJ Sokol Husovice']);
    });
