...
it blocks the alley (úklid child + cancelled reservations, exactly as when
entered in the app). In every other row a match is taken only when one of
...
def build_sql(matches: List[Match], tenant: str, prep: int, source: str) -> str:
    ...
    return '\n'.join([
        ...
        "-- Runs as the alley's admin (RLS, úklid children and cancelled reservations as in",
        '-- the app). Safe to re-run: rows are keyed by import_key and never inserted twice.',
        'begin;',
